...
import { Search, ShieldAlert, CheckCircle2, Mail, ArrowRight, ArrowLeft, Loader2 } from 'lucide-react';
...
export const RegisterForm: React.FC = () => {
  ...
  const handleLookupSubmit = (e: React.FormEvent) => {
    ...
    handleIdentityLookup(searchId);
  };

  // Step 3 Submission Handler (Credential Setup)
  const handleCredentialSubmit = (e: React.FormEvent) => {
    ...
    // Redirect to the Email Verification / OTP screen step
    window.location.href = `/auth/email-verification?email=${encodeURIComponent(email)}&id=${encodeURIComponent(searchId)}`;
  };
        ...
        <div className="space-y-6">
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-400 dark:text-slate-500 font-normal">
              Provide your official registration identifier to scan and verify your records.
            </p>
          </div>
          ...
          <form onSubmit={handleLookupSubmit} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <p className="text-[10px] text-slate-400 italic leading-relaxed">
                * Note: Freshmen utilize official JAMB formatting to parse attributes. Returning streams apply Matric IDs.
              </p>
            </div>
          ...
          <div className="text-center pt-2 border-t border-slate-100 dark:border-slate-900">
            <p className="text-[11px] text-slate-400">
              Already initialized? <a href="/auth/login" className="text-slate-900 dark:text-white font-bold underline">Log In here</a>
            </p>
          </div>
        </div>
      )}
      ...
      {registrationStep === 'CONFIRM_PROFILE' && discoveredProfile && (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-200">
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-400 dark:text-slate-500 font-normal">
              The provided identifier matches the secure institutional system record mapped below.
            </p>
          </div>
          ...
          <div className="bg-slate-50 dark:bg-slate-950 rounded-xl p-5 border border-slate-200/60 dark:border-slate-900 grid grid-cols-2 gap-y-4 gap-x-2 text-xs">
            <div className="col-span-2 border-b border-slate-200/40 dark:border-slate-900 pb-2 flex justify-between items-center">
              <span className="text-[10px] font-mono uppercase text-slate-400">Database Reference Token</span>
              <span className="font-mono text-slate-900 dark:text-slate-200 font-bold">{discoveredProfile.id}</span>
            </div>
            <div>
              <span className="text-[10px] font-black uppercase text-slate-400 tracking-wider block mb-0.5">First Name</span>
              <span className="font-bold text-slate-900 dark:text-white text-sm">{discoveredProfile.firstName}</span>
            </div>
            <div>
              <span className="text-[10px] font-black uppercase text-slate-400 tracking-wider block mb-0.5">Last Name</span>
              <span className="font-bold text-slate-900 dark:text-white text-sm">{discoveredProfile.lastName}</span>
            </div>
            <div className="col-span-2">
              <span className="text-[10px] font-black uppercase text-slate-400 tracking-wider block mb-0.5">Faculty Allocation</span>
              <span className="font-semibold text-slate-700 dark:text-slate-300">{discoveredProfile.faculty}</span>
            </div>
            <div className="col-span-2">
              <span className="text-[10px] font-black uppercase text-slate-400 tracking-wider block mb-0.5">Department / Discipline</span>
              <span className="font-semibold text-slate-700 dark:text-slate-300">{discoveredProfile.department}</span>
            </div>
            <div>
              <span className="text-[10px] font-black uppercase text-slate-400 tracking-wider block mb-0.5">Assigned Cohort Level</span>
              <span className="font-semibold text-slate-700 dark:text-slate-300">{discoveredProfile.level}</span>
            </div>
            <div>
              <span className="text-[10px] font-black uppercase text-slate-400 tracking-wider block mb-0.5">Active Session</span>
              <span className="font-semibold text-slate-700 dark:text-slate-300">{discoveredProfile.academicYear}</span>
            </div>
          </div>
      ...
      {registrationStep === 'CREATE_ACCOUNT' && (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-200">
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-400 dark:text-slate-500 font-normal">
              Configure your primary communications email and encrypted access passphrase keys.
            </p>
          </div>
          ...
          <form onSubmit={handleCredentialSubmit} className="space-y-4">
            ...
            <div className="flex gap-4 pt-2">
              <button
                type="button"
                onClick={() => advanceToCredentialCreation()} // Returns visually back to confirmation matrix
                className="w-1/3 h-12 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-700 dark:text-slate-300 text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-1.5 hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors cursor-pointer"
              >
                Back
              </button>
